...
file takes a comparator `cmp` as input
...
def _merged(xs, ys, cmp=cmp_standard):
    ...
    as a local function within the
    ...
    Typically, you should only implement
    a function as a local function if
    ...
    and write test cases for the stand-alone function.
    >>> _merged([1, 3, 5], [2, 4, 6])
    [1, 2, 3, 4, 5, 6]
    '''
    ixs = 0
...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    and the list is split into
    a "less than" sublist and a "greater than" sublist.
    The pseudocode is:
        if xs has 1 element
            it is sorted, so return xs
        else
...
def quick_sort(xs, cmp=cmp_standard):
    ...
    this more efficient memory usage typicall
    y makes quick sort faster in practice.
    (We say quick sort has a lower "constant factor" in its runtime.)
    The downside of implementing quick sort in
    this way is that it will no longer be a
    ...
    xs variable instead of returning a copy of
    the list.
    '''
